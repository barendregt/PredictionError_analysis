...
def fit_hddm_model(trace_id, data, model_dir, model_name, samples=10000):
	...
	m.sample(samples, burn=samples/10, thin=5, dbname=os.path.join(model_dir, model_name+ '_db{}'.format(trace_id)), db='pickle')
...
def fit_reg_model(trace_id, data, model_spec, model_dir, model_name, samples=10000):
	...
	m = hddm.models.HDDMRegressor(data, model_spec,include=('sv'), p_outlier = 0.05)
...
# model_name = 'RT_and_pupil-only_v'
# model_spec = ["v ~ 0+C(trial_PE)*pupil_response_1"]
