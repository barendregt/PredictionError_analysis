...
for subname in sublist:
	...
	# 	power_signal = np.dot(trial_signal, msignal)/(msignal.size**2)#(np.linalg.norm(msignal, ord=2)**2)
	...
	# psignals_std  = [np.std(s) for s in psignals]
	...
	plt.bar(np.arange(3), rts_mean[1:], yerr = rts_std[1:], color='w')
	plt.axis('tight')
	plt.xticks(np.arange(3)+0.25, ('P-U','U-P','U-U'))
...
plt.savefig(os.path.join(figfolder,'all-ev_pupil-rt.pdf'))
