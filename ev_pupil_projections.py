	...
	try:
		for key, trial_signal in pa.trial_signals.items():
			...
			if key < 10:
				...
				response_diff_signals['PU'].extend(np.dot(trial_signal,np.mean(all_pupil_signals,axis=0))/np.linalg.norm(np.mean(all_pupil_signals,axis=0), ord=2)**2)
			else:
				response_diff_signals['UU'].extend(np.dot(trial_signal,np.mean(all_pupil_signals,axis=0))/np.linalg.norm(np.mean(all_pupil_signals,axis=0), ord=2)**2)
	except:
		embed()
...
# pl.save_figure('pupil_amplitude_button-press.pdf', sub_folder = 'over_subs')
	...
	pl.open_figure(force=1)
	pl.bar_plot(data = response_diff_signals, conditions = ['PP','UP','PU','UU'], ylabel='Pupil response', with_error = True)
	pl.save_figure('pupil_projection.pdf', sub_folder='per_sub/power') 
	# pl.open_figure(force=1)
	# pl.hline(y=0)
	# pl.event_related_pupil_average(data = response_diff_signals, conditions = ['PP','UP','PU','UU'], signal_labels = {'PP': 'Predicted', 'UP': 'Task relevant','PU':'Task irrelevant','UU':'Both'}, show_legend=True, ylabel = 'Pupil size', x_lim = [0.5*(signal_sample_frequency/down_fs), 4.5*(signal_sample_frequency/down_fs)], xticks = np.arange(0,5*(signal_sample_frequency/down_fs),0.5*(signal_sample_frequency/down_fs)), xticklabels = np.arange(response_deconvolution_interval[0], response_deconvolution_interval[1],.5), y_lim = [-.3, .8], compute_mean = True, compute_sd = True)

	# pl.save_figure('pupil_project_button-press.pdf', sub_folder = 'per_sub/pupil')
	...
	# pl.save_figure('inc_pupil_response-stimulus.pdf', sub_folder = 'per_sub/pupil')
