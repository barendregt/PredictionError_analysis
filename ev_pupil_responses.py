	...
	try:
		for key, trial_signal in pa.trial_signals.items():
			...
			if len(trial_signal)>0:
				...
				if key < 10:
					response_diff_signals['PP'].append(np.mean(trial_signal - ref_signals.mean(axis=0), axis=0))
				elif key < 30:
					response_diff_signals['PU'].append(np.mean(trial_signal - ref_signals.mean(axis=0), axis=0))
				elif key <50:
					response_diff_signals['UP'].append(np.mean(trial_signal - ref_signals.mean(axis=0), axis=0))
				else:
					response_diff_signals['UU'].append(np.mean(trial_signal - ref_signals.mean(axis=0), axis=0))
	except:
		embed()
