	...
	try:
		for key, trial_signal in pa.trial_signals.items():
			...
			if len(trial_signal)>0:
				...
				if key < 10:
					response_diff_signals['PP'].extend(trial_signal - np.mean(ref_signals, axis=0), axis=0)
				elif key < 30:
					response_diff_signals['PU'].extend(trial_signal - np.mean(ref_signals, axis=0), axis=0)
				elif key <50:
					response_diff_signals['UP'].extend(trial_signal - np.mean(ref_signals, axis=0), axis=0)
				else:
					response_diff_signals['UU'].extend(trial_signal - np.mean(ref_signals, axis=0), axis=0)
	except:
		embed()
