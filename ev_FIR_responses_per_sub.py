...
for subname in sublist:
	...
	# try:
	betas = pa.get_IRF_correct_incorrect()
	# except:
	# 	embed()
	...
	response_fir_signals['color']['PU'].append(betas[1][:,0])
	response_fir_signals['color']['PP'].append(betas[1][:,1])
	response_fir_signals['color']['UU'].append(betas[1][:,2])
	response_fir_signals['color']['UP'].append(betas[1][:,3])
...
plt.savefig(os.path.join(figfolder,'over_subs','FIR_comb_oc.pdf'))
